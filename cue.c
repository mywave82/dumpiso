...
static enum cdfs_format_t cue_track_mode_to_cdfs_format (enum cue_track_mode_t mode, int SWAP)
{
	switch (mode)
	{
		case AUDIO:      return SWAP ? FORMAT_AUDIO_SWAP___NONE : FORMAT_AUDIO___NONE;
...
struct cdfs_disc_t *cue_parser_to_cdfs_disc (const char *argv1_path, struct cue_parser_t *cue_parser)
{
	...
	if (!retval)
	{
		fprintf (stderr, "cue_parser_to_cdfs_disc(): calloc() failed\n");
		return 0;
	}
	...
	for (i=0; i < cue_parser->datasourceN; i++)
	{
		...
		#warning REMOVE-ME
		fprintf (stderr, "cue_parser = %p, i=%d, datasourcesN=%d\n", cue_parser, i, cue_parser->datasourceN);

		if (cue_parser->datasource[i].wave)
		{
			if (wave_openfile (argv1_path, cue_parser->datasource[i].filename, &fd, &offset, &length))
			{
				fprintf (stderr, "Failed to open wave file %s (format must be stereo, 16bit, 44100 sample-rate)\n", cue_parser->datasource[i].filename);
				goto fail_out;
			}
		} else {
			if (data_openfile (argv1_path, cue_parser->datasource[i].filename, &fd, &length))
			{
...
struct cue_parser_t *cue_parser_from_fd (int fd)
{
	...
	return cue_parser_from_data (buffer);
}
...
int main (int argc, char *argv[])
{
	...
	cue = cue_parser_from_filename (argv[1]);
	if (cue)
	{
